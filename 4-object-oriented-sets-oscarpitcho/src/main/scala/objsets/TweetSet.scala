...
abstract class TweetSet extends TweetSetInterface {
  ...
  /**
    * Auxiliary method for the mostRetweeted method
    */
  def mostRetweetedAux(x: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  def mostRetweetedAux(x: Tweet): Tweet = x
}
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    val maxRetweeted: Tweet = mostRetweeted
    new Cons(maxRetweeted, (this remove maxRetweeted) descendingByRetweet)
  }

  def mostRetweetedAux(x: Tweet): Tweet = {
    def compare(t1: Tweet, t2: Tweet): Boolean = {
      (t1.retweets > t2.retweets)
    }
    val lTweet: Tweet = left mostRetweetedAux x
    val rTweet: Tweet = right mostRetweetedAux x
    if (compare(elem, lTweet) && compare(elem, rTweet)) elem
    else if (compare(rTweet, lTweet)) rTweet
    else lTweet
  }
