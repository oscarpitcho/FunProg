...
trait Huffman extends HuffmanInterface {
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    def insert(l: List[CodeTree], elem: CodeTree, acc: List[CodeTree]): List[CodeTree] = {
      if (l.isEmpty) acc :+ elem;
      else if (weight(elem) < weight(l.head)) (acc :+ elem) ++ l
      else insert(l.tail, elem, acc :+ l.head)
    }
    if (trees.isEmpty || singleton(trees)) trees
    else {
    val l = trees.tail.head
    val r = trees.head
    val combined: CodeTree =
      new Fork(r, l, chars(r) ++ chars(l), weight(r) + weight(l))
    insert(trees.tail.tail, combined, Nil)
    }
  }
  ...
  /**
    ...
    */
  def until(endFunc: List[CodeTree] => Boolean,
            stepFunc: List[CodeTree] => List[CodeTree])(
      l: List[CodeTree]): List[CodeTree] = {
    if (endFunc(l)) l
    else until(endFunc, stepFunc)(stepFunc(l))
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    def aux(currT: CodeTree, b: List[Bit], acc: List[Char]): List[Char] = {
      currT match {
        case Leaf(char, _) =>
          if (b.isEmpty) acc :+ char else aux(tree, b, acc :+ char);
        case Fork(l, r, _, _) =>
          if (b.head == 1) aux(r, b.tail, acc) else aux(l, b.tail, acc)
      }
    }
    aux(tree, bits, Nil)
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def aux(currT: CodeTree, txt: List[Char], acc: List[Bit]): List[Bit] = {
      if (txt.isEmpty) acc
      else
        currT match {
          case Fork(l, r, c, _) =>
            if (chars(l).contains(txt.head)) aux(l, txt, acc :+ 0)
            else aux(r, txt, acc :+ 1)
          case Leaf(c, _) => aux(tree, txt.tail, acc)
        }
    }
    aux(tree, text, Nil)
  }
  ...
  def convert(tree: CodeTree): CodeTable = {
      tree match { 
        ...
        case Leaf(c, _) => List[(Char, List[Bit])]{(c, Nil)};
      }
  ...
  def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    def aux(table: CodeTable)(text: List[Char], acc: List[Bit]): List[Bit] = {
      if(text.isEmpty) acc
      else aux(table)(text.tail, acc ++ codeBits(table)(text.head))
    }
