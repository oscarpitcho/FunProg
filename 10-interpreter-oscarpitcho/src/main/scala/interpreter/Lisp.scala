...
// Environments -------------------------------------------------------
  ...
  object EmptyEnvironment extends Environment {
    def lookup(n: String): Data =
      sys.error(
        "undefined: " + n + ", this can happen when the wrong number of arguments is passed")
  }
  ...
  def eval1(x: Data, env: Environment): Data = 
    x match {
      ...
      case 'def :: (name :: args) :: body :: rest :: Nil =>
        val function = List('name, args, body)
        eval(List('def, name, List('lambda, args, body), rest), env)

      case 'case :: expr :: conditions =>
        // println('case + " expr =  " + expr + " conditions = " + conditions)
        conditions match {
          case List('else :: exp :: Nil) => println("In else"); eval1(exp, env);
          case (cond :: exp :: Nil) :: rest =>
            eval1(List('if,
                       List('=, cond, expr),
                       exp,
                       eval1('case :: expr :: rest, env)),
                  env)
          case _ =>
            sys.error(
              "Invalid case exp: " + 'case + " expr =  " + expr + " conditions = " + conditions)
        }
      ...
      case 'def :: Symbol(name) :: body :: Nil => // definition GLOBAL
        if (env == globalEnv) {
          ...
          sys.error(
            s"def $name cannot be added to an inner scope because it is global.\n" +
              "Hint: a lvarocal def always has three parameters: a name, a body, and the rest of the code to execute")
        }
